...
class ItemCraft_Scene
  ...
  def pbCraftItem(stock)
    ...
    loop do
      ...
      if Input.trigger?(Input::USE)
        ...
        pocket = item.pocket
        if pbConfirmMessage(_INTL("\\c[1]Would you like to craft {1} {2}?",volume,itemname))
          if canCraft?(index,volume)
            ...
            pbPlayBuzzerSE
            pbMessage(_INTL("\\c[2]ERROR! \\c[1]Not enough ingredients."))
          end
        end
      end
  ...
  def pbRedrawItem(index,volume)
    ...
    num = @stock[index][1].length/2
    textpos = [
    ["C: Use",4,Graphics.height-38,0,BASELIGHT,SHADOWLIGHT],
    ["ARROWS: Navigate",Graphics.width/2,Graphics.height-38,2,BASELIGHT,SHADOWLIGHT],
    ["X: Cancel",Graphics.width-4,Graphics.height-38,1,BASELIGHT,SHADOWLIGHT]
    ]
...
def pbItemCrafter(stock,speech1=nil,speech2=nil)
  ...
  if pbMessage(_INTL("\\c[1]Hyper crafter activited!"))
    pbMessage(speech1 ? speech1 : _INTL("\\c[1]Crafting recipe activited!"))
    pbFadeOutIn {
  ...
  pbMessage(speech2 ? speech2 : _INTL("\\c[1]Hyper crafter shuting down..."))
end



#("Would you like to Craft an Item?"))