...
class LocationWindow
  ...
  def initialize(name)
    ...
    for name in STATIONARY
      ...
      s=name.gsub(/^./) { |m| m.upcase }
      newarr9.push(s)
      stationary=STATIONARY+newarr10
    end
    #
    newarr11=[]
    for name in CASTLELIZE
      next if !name.is_a?(String)
      s=name.gsub(/^./) { |m| m.upcase }
      newarr9.push(s)
      castlelize=CASTLELIZE+newarr11
    end
    #=================================================================================================#
    #                                                                                                 #
    #=================================================================================================# 
    ...
    for i in 0..theta.length-1
      if $game_map.name.include?(theta[i]) || @currentmap==THETA[i]
         @window.bitmap=RPG::Cache.load_bitmap("Graphics/Pictures/Location/","theta")
      end
    end
    #
    for i in 0..stationary.length-1
      if $game_map.name.include?(stationary[i]) || @currentmap==STATIONARY[i]
          @window.bitmap=RPG::Cache.load_bitmap("Graphics/Pictures/Location/","stationary")
      end
    end
    #
    for i in 0..castlelize.length-1
      if $game_map.name.include?(castlelize[i]) || @currentmap==CASTLELIZE[i]
          @window.bitmap=RPG::Cache.load_bitmap("Graphics/Pictures/Location/","castlelize")
      end
    end
    #=================================================================================================#
    #                                                                                                 #
