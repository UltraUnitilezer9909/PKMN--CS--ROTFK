...
def pbEmergencySave
  ...
  if Game.save
    pbMessage(_INTL("\\se[]The \\c[{1}]game \\c[{2}]was \\c[{1}]saved.\\me[GUI save game] \\c[{2}]The \\c[{3}]previous save file \\c[{2}]has been \\c[{4}]backed up.\\wtnp[30]", 6, 0, 2, 1)) #pbMessage(_INTL("\\se[]The game was saved.\\me[GUI save game] The previous save file has been backed up.\\wtnp[30]"))
  else
    pbMessage(_INTL("\\c[2]\\se[]Save failed.\\wtnp[30]")) #pbMessage(_INTL("\\se[]Save failed.\\wtnp[30]"))
  end
  $scene = oldscene
end
...
class PokemonSaveScreen
  ...
  def pbSaveScreen
    ret=false
    @scene.pbStartScreen
    if pbConfirmMessage(_INTL("Would you like to \\c[6]save\\c[0] the \\c[6]game\\c[0]?")) #if pbConfirmMessage(_INTL("Would you like to save the game?"))
#    if pbConfirmMessageSystemModern(0,false,_INTL("Would you like to save the game?"))
      if safeExists?(RTP.getSaveFileName("Game.rxdata"))
        if $PokemonTemp.begunNewGame
          ...
          pbMessage(_INTL('\\c[{1}]The game can only hold one save file...',2)) #pbMessage(_INTL("There is a different game file that is already saved."))
          ...
          if !pbConfirmMessageSerious(
            _INTL("Are you \\c[{1}]sure \\c[{2}]you want to \\c[{1}]save \\c[{2}]now and \\c[{3}]overwrite\\c[{2}] the other \\c[{1}]save file\\c[0]?",6 ,0 ,2 , 1)) #_INTL("Are you sure you want to save now and overwrite the other save file?"))
      ...
      pbSEPlay("GUI save choice")
      if pbSave
        pbMessage(_INTL("\\se[]\\c[1]{1} \\c[0]saved the \\c[6]Game.\\me[GUI save game]\\wtnp[20]", $Trainer.name)) #pbMessage(_INTL("\\se[]{1} saved the game.\\me[GUI save game]\\wtnp[30]",$Trainer.name))
        ret=true
      else
        pbMessage(_INTL("\\c[2]\\se[]Save failed.\\wtnp[30]")) #pbMessage(_INTL("\\se[]Save failed.\\wtnp[30]"))
        ret=false
